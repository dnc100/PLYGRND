  ...
  return (
    <div className="min-h-[calc(100vh-80px)] w-full bg-white text-black font-mono p-4 md:p-8 grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-8 selection:bg-black selection:text-white screen-only">
      ...
      <div className="bg-[#f4f4f0] border-4 border-black p-4 md:p-6 flex flex-col gap-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] screen-only overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4 mt-2">
            <div>
              ...
              <input
                ...
                className="w-full bg-[#f4f4f0] border-2 border-black p-2 font-bold text-lg focus:outline-none focus:bg-black focus:text-white transition-colors"
              />
            </div>
            <div>
              <label className="text-[10px] font-bold uppercase block mb-1">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-3 mb-2">
            {presets.map((p) => (
              <button
                ...
                className="w-8 h-8 border-2 hover:scale-110 transition-transform shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                style={{
                  backgroundColor: p.bg,
                  borderColor: p.border === '#FFFFFF' ? '#000000' : p.border,
                }}
                title={p.name}
              />
            ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="flex flex-col">
              <label className="text-[8px] font-bold uppercase mb-1">
                Canvas
              </label>
              <input
                type="color"
        ...
        {activeCell ? (
          <div className="border-4 border-black p-4 bg-[#FFEA00] relative">
            <div className="absolute -top-3 left-2 bg-white px-2 text-[10px] font-black uppercase border-2 border-black">
              Edit Cell [{activeCell}]
            </div>
            <button
              onClick={() => setActiveCell(null)}
              className="absolute top-2 right-2 text-xl font-black leading-none hover:text-white"
            >
              ✕
            </button>
            ...
            <div className="mt-4 flex flex-col gap-4">
              <div className="flex gap-2">
                <div className="w-1/2">
                  ...
                  <input
                    ...
                    min="1"
                    max={columns}
                    value={cellData[activeCell]?.colSpan || 1}
                    onChange={(e) =>
                      handleCellChange(
                        activeCell,
                        'colSpan',
                    ...
                    className="w-full border-2 border-black p-1 font-bold text-center"
                  />
                </div>
                <div className="w-1/2">
                  <label className="text-[10px] font-bold uppercase block">
                  ...
                  <input
                    ...
                    min="1"
                    max={rows}
                    value={cellData[activeCell]?.rowSpan || 1}
                    onChange={(e) =>
                      handleCellChange(
                        activeCell,
                        'rowSpan',
              ...
              <div>
                <select
                  value={cellData[activeCell]?.type || 'text'}
                  onChange={(e) =>
                    handleCellChange(activeCell, 'type', e.target.value)
                  }
                  className="w-full p-2 border-2 border-black font-bold uppercase text-xs cursor-pointer"
                >
              ...
              {!cellData[activeCell]?.type ||
              cellData[activeCell]?.type === 'text' ? (
                <>
                  <textarea
                    value={cellData[activeCell]?.text || ''}
                    onChange={(e) =>
                      handleCellChange(activeCell, 'text', e.target.value)
                    }
                    placeholder="TYPE HERE..."
                    rows="3"
                    className="w-full p-2 border-2 border-black text-sm font-sans resize-none focus:outline-none"
                  />

                  <div>
                    <label className="text-[10px] font-bold uppercase block mb-1">
                      Text Size ({cellData[activeCell]?.fontSize || 16}px)
                    </label>
                    <input
                      type="range"
                      min="10"
                      max="120"
                      value={cellData[activeCell]?.fontSize || 16}
                      onChange={(e) =>
                        handleCellChange(
                          activeCell,
                          'fontSize',
                ...
                <div className="border-2 border-black bg-white p-2">
                  <label className="text-[10px] font-bold uppercase block mb-2 cursor-pointer hover:underline">
                    ...
                    <input
                      ...
                      accept="image/*"
                      className="hidden"
                      onChange={(e) => handleImageUpload(e, activeCell)}
                    />
                  </label>
                  <p className="text-[8px] text-gray-500 break-all">
                    {cellData[activeCell]?.imgUrl
                      ? 'Image Loaded.'
                      : 'No file selected.'}
                  </p>
                </div>
          ...
          <div className="border-4 border-black border-dashed p-8 text-center text-gray-400 font-bold uppercase text-xs">
            Select a cell on the canvas to inject content
          </div>
        )}

        <button
          onClick={handleExport}
          className="brutal-btn mt-auto w-full bg-[#FF3300] text-black py-4 text-sm font-black tracking-widest uppercase"
        >
          Print / Export
        </button>
      </div>
      ...
      <div className="w-full h-[calc(100vh-8rem)] md:h-[calc(100vh-8rem)] flex justify-center items-center overflow-auto p-4 md:p-8 bg-zinc-100 border-2 border-black inset-shadow">
        <div
          ref={posterRef}
          className="print-area shadow-[16px_16px_0px_0px_rgba(0,0,0,0.1)] transition-colors relative overflow-hidden flex-shrink-0"
          style={{
        ...
        >
          <div
            className="w-full h-full grid grid-flow-row-dense"
            style={{
            ...
            {gridCells.map((key) => {
              ...
              return (
                <div
                  ...
                  className={`relative group cursor-pointer overflow-hidden flex flex-col transition-transform ${
                  ...
                  style={{
                    ...
                    borderWidth: '2px',
                    borderStyle: 'solid',
                    borderColor: isSelected ? borderColor : `${borderColor}66`,
                  }}
                >
                  <div className="w-full h-full flex p-3 pointer-events-none z-10 relative">
                    {!current?.type || current.type === 'text' ? (
                      <p
                        className="font-sans font-bold leading-none tracking-tight whitespace-pre-wrap break-words"
                  ...
                  {!current?.text && !current?.imgUrl && (
                    <span className="screen-only absolute inset-0 flex items-center justify-center text-[10px] font-black opacity-30 text-current mix-blend-difference">
                      +
                    </span>
                  )}
                </div>
...
export default function CustomInteractiveButton() {
  ...
  return (
    <button
      ...
      style={{
        ...
        animation: '${
          config.anim === 'breathe' && !isHovered
            ? 'breathe 3s infinite ease-in-out'
            : 'none'
        }',
      }}
    >
      {isHovered && '${config.anim}' === 'glitch' ? '${config.label.replace(
      /.$/,
      '@'
    )}' : '${config.label}'}
    </button>
  ...
  useEffect(() => {
    const sketch = (p) => {
      ...
      p.setup = () => {
        p.createCanvas(p.windowWidth > 800 ? 600 : 340, 400);
        p.rectMode(p.CENTER);
        ...
        for (let i = 0; i < NUM_NODES; i++) {
          blobNodes.push({
            angle: p.map(i, 0, NUM_NODES, 0, p.TWO_PI),
            ox: 0,
            oy: 0,
          });
        }
      ...
      const drawBlob = (xOff, yOff, width, height, color, isShadow) => {
        ...
        for (let i = 0; i < NUM_NODES + 3; i++) {
          ...
          p.curveVertex(
            rX * p.cos(angle) + node.ox,
            rY * p.sin(angle) + node.oy
          );
        }
        p.endShape();
        p.pop();
      ...
      p.draw = () => {
        const { shape, anim, buttonColor, textColor, label } =
          configRef.current;
        p.clear();
        time += 0.05;
        ...
        if (
          p.mouseX > centerX - baseWidth / 2 &&
          p.mouseX < centerX + baseWidth / 2 &&
          p.mouseY > centerY - baseHeight / 2 &&
          p.mouseY < centerY + baseHeight / 2
        ) {
          if (!isHovered) {
            p.cursor(p.HAND);
        ...
        if (anim === 'breathe') {
          widthMod = p.sin(time) * 10;
          heightMod = p.cos(time) * 5;
        } else if (anim === 'glitch' && isHovered) {
          xOffset = p.random(-4, 4);
          yOffset = p.random(-4, 4);
          if (p.random(1) > 0.8) widthMod = p.random(-20, 20);
        } else if (anim === 'elastic') {
          widthMod = isHovered ? p.sin(time * 3) * 15 : 0;
        }
        ...
        if (shape === 'blob') {
          for (let i = 0; i < NUM_NODES; i++) {
            ...
            let nx = p.cos(node.angle) + 1;
            let ny = p.sin(node.angle) + 1;
            ...
            if (p.mouseIsPressed && isHovered) {
              let dx = p.mouseX - absX;
              let dy = p.mouseY - absY;
            ...
            node.ox += (0 - node.ox) * 0.15;
            node.oy += (0 - node.oy) * 0.15;
          }
        }
        ...
        if (shape === 'blob') {
          drawBlob(6, 6, finalWidth, finalHeight, '#000000', true);
        } else {
          p.fill(0);
          p.noStroke();
          if (shape === 'circle') p.circle(6, 6, finalWidth);
          else if (shape === 'pill')
            p.rect(6, 6, finalWidth, finalHeight, finalHeight / 2);
          else p.rect(6, 6, finalWidth, finalHeight);
        }

        if (shape === 'blob') {
          drawBlob(0, 0, finalWidth, finalHeight, buttonColor, false);
        } else {
          p.fill(buttonColor);
          p.stroke(0);
          p.strokeWeight(4);
          if (shape === 'circle') p.circle(0, 0, finalWidth);
            ...
            p.rect(0, 0, finalWidth, finalHeight, finalHeight / 2);
        ...
        if (anim === 'glitch' && isHovered && p.random(1) > 0.7) {
          p.text(
      ...
      p.windowResized = () => {
        p.resizeCanvas(p.windowWidth > 800 ? 600 : 340, 400);
      };
    };
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] w-full bg-[#f4f4f0] text-black font-mono p-4 md:p-8 grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-8 selection:bg-black selection:text-white">
      <div className="bg-white border-4 border-black p-6 flex flex-col gap-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] z-10 relative overflow-y-auto">
        <div className="border-b-4 border-black pb-4">
          ...
          <p className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">
            p5.js Physics & React Export
          </p>
        </div>

        <button
          onClick={randomizeAll}
          className="w-full bg-[#FFEA00] border-4 border-black p-3 font-black uppercase text-sm hover:-translate-y-1 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all"
        >
          Generate Random
        </button>
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-2">
              {shapes.map((s) => (
                <button
                  ...
                  className={`flex-1 border-2 border-black p-2 text-xs font-bold uppercase ${
                    config.shape === s
                      ? 'bg-black text-white'
          ...
          <div>
            ...
            <div className="flex gap-2">
              {anims.map((a) => (
                <button
                  key={a}
                  onClick={() => setConfig({ ...config, anim: a })}
                  className={`flex-1 border-2 border-black p-2 text-xs font-bold uppercase ${
                    config.anim === a
                      ? 'bg-black text-white'
                      : 'bg-white text-black hover:bg-zinc-100'
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col">
                ...
                <input
                  ...
                  className="w-full h-8 border-2 border-black cursor-pointer p-0"
                />
              </div>
              <div className="flex flex-col">
                <label className="text-[8px] font-bold uppercase mb-1">
          ...
          <div>
            ...
            <input
              ...
              value={config.label}
              onChange={(e) =>
                setConfig({ ...config, label: e.target.value.toUpperCase() })
              }
              maxLength={12}
              className="w-full bg-zinc-100 border-2 border-black p-2 font-bold focus:outline-none focus:bg-black focus:text-white transition-colors uppercase"
            />
          </div>
        </div>
        ...
        <button
          ...
          className={`mt-auto w-full border-4 border-black p-4 font-black uppercase tracking-widest text-sm transition-all ${
            copied
              ? 'bg-black text-white'
              : 'bg-white text-black hover:bg-zinc-100'
          }`}
      ...
      <div className="w-full h-[calc(100vh-8rem)] md:h-[calc(100vh-8rem)] bg-zinc-200 border-2 border-black flex justify-center items-center inset-shadow relative overflow-hidden">
        ...
        <div ref={canvasRef} className="z-10 shadow-2xl" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col font-mono selection:bg-white selection:text-black">
      ...
      <header className="h-[80px] bg-[#FFEA00] border-b-4 border-black flex items-center justify-between px-4 md:px-8 shrink-0 z-50">
        ...
        <div className="flex gap-2 md:gap-4">
          <button
            ...
            className={`px-3 md:px-6 py-2 border-2 border-black font-bold uppercase text-[10px] md:text-sm transition-all ${
              activeTab === 'poster'
      ...
      <main className="flex-1 overflow-hidden bg-zinc-100 relative">
        {activeTab === 'poster' ? (
          <BrutalistPosterCreator />
        ) : (
          <InteractiveComponentFoundry />
        )}
      </main>
    </div>
  );
